  ...
  bind_rows({
    completed_transport_projects %>%
      ...
      summarise(first_estimate = first(Cost_estimate_real, order_by = Record_date), 
                final_estimate = last(cost_when_completed_real)) %>%
      ungroup %>%
      merge(cost_overruns_by_GR) %>%
      mutate(phase_cost_overrun = final_estimate - first_estimate, 
             phase_cost_overrun_pc = phase_cost_overrun / first_cost_estimate_real) %>%
      ungroup %>%
...
cost_overruns_by_GR_phase_unique<-
  cost_overruns_by_GR_phase%>%
  filter(!is.na(change_was_from)) %>%
  setkey(Grattan_record, change_was_from) %>%
  unique
