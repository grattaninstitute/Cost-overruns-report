...
savings_if_every_proj_with_CO_had_CO_le_10pc <-
  ...
  mutate(overrun_threshold = 0.1 * first_cost_estimate_real, 
         overrun_cf_capped = cost_overrun - overrun_threshold) %>%
...
Cost overruns in \Sexpr{grattan_percent(prop_projects_w_overrun)} of the transport infrastructure budget have cost taxpayers \Sexpr{texNum(total_cost_overruns, dollar = TRUE)} over the past 15 years. 
...
cost_overrun_by_GR_phase <-
  ...
  setkey(Grattan_record) %>%
  merge(cost_overrun_by_GR) %>%
  select(Grattan_record, change_was_from, phase_cost_overrun, project_cost_overrun = cost_overrun)
...
overruns_during_construction_as_prop_of_final_value <- 
{
  ...
  merge(cost_overrun_during_construction_by_GR, cost_overrun_by_GR, by = "Grattan_record") %>%
    filter(!outlier) %$%
...
Premature announcement affects not only the likelihood of an overrun, but also its size. Projects with costs announced ahead of a government funding commitment have far larger cost overruns, averaging \Sexpr{grattan_percent(overrun_pc_if_premature_announcment)} of the initially announced cost (Figure xx).  Figure xx illustrates net overruns of \Sexpr{round((overall_overruns_by_cohort[2,4]-1)*100,0)} to \Sexpr{round((overall_overruns_by_cohort[3,4]-1)*100,0)} per cent occur for projects announced with more mature cost estimates than those announced early.
