...
<<cost_when_completed>>=
# Note: some of these projects were deleted, not completed.
cost_when_completed <-
  historical_dataset %>%
  as.data.table %>%
  mutate(Grattan_record = coalesce(`Project No.`), cost_when_completed = `Total cost $m`*10^6) %>%
  setkey(Grattan_record) %>%
...
important_vars_by_GR %>%
  ...
  logitmfx(factor(cost_overrun > 0) ~ important_vars_by_GR$premature_announcement + 
          log_Total_days_pre_construction + 
          big_states + 
          Sub_industry + 
...
<<fit_logit_co_all_t1>>=
# This regression (t1) considers the cost that occurred over the possible_or_under_cosideration stage (between first possible_or_under_consideration and first committed cost)
...
cost_overruns_by_GR_phase <-
  ...
  mutate(phase_cost_overrun = coalesce(first_cost_change, 0), 
         phase_cost_overrun_pc_of_initial_phase_cost = phase_cost_overrun / first_cost, 
