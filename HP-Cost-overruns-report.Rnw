...
first_status_by_GR <-
  ...
  mutate(first_pipeline_status = first(Status_ordered, order_by = Record_date)) %>%
  setkey(Grattan_record) %>%
  unique() %>%
  select(Grattan_record, first_pipeline_status)
...
# ISSUES: historical dataset \neq completed projects list
# Can't get rid of all the NA's in completed project list's Final_status_historical
historical_dataset_working_copy <-
historical_dataset %>%
  filter(`Major industry` == "Transport & Storage",
         `Sub-industry` %in% c("Road", "Rail")) %>% 
...
historical_dataset_working_copy %>%
  setkey(Grattan_record) %>% unique %>% nrow

transport_projects %>% setkey(Grattan_record) %>% unique %>% nrow

completed_project_list %>% as.data.table %>% setkey(Grattan_record) %>% unique %>% nrow

transport_projects %>%
  merge(historical_dataset_working_copy) %>%
  unique %>% nrow

transport_projects %>%
  merge(completed_project_list) %>%
  unique %>% nrow
...
final_status_by_GR <- 
  ...
  group_by(Grattan_record) %>%
  mutate(final_pipeline_status = last(Status_ordered, order_by = Record_date)) %>%
  setkey(Grattan_record) %>%
  unique() %>%
  select(Grattan_record,  final_pipeline_status, Final_status_historical) 


final_status_by_GR_no_NAs <-
  final_status_by_GR %>%
  mutate(Final_status_historical =  if_else(is.na(Final_status_historical) & as.character(final_pipeline_status) == "Possible or under considertaion", 
                                            "Deleted", Final_status_historical),
         Final_status_historical = if_else(is.na(Final_status_historical) & final_pipeline_status == "Committed", "Deleted", Final_status_historical),
         Final_status_historical =  if_else(is.na(Final_status_historical) & final_pipeline_status=="Under construction", 
                                            "Completed", Final_status_historical), 
         Final_status_historical = if_else(is.na(Final_status_historical), as.character(final_pipeline_status), Final_status_historical)) %>%
  select(Grattan_record, Final_status_historical)


table((final_status_by_GR_no_NAs$Final_status_historical))


table((final_status_by_GR$final_pipeline_status))
...
<<sample_sizes_by_historical_status>>=
sample_size <-
  transport_projects %>% 
  setkey(Grattan_record) %>%
  unique() %>% nrow()
...
sample_size_completed <-
  ...
  merge(final_status_by_GR) %>% 
  mutate(total_sample_size = nrow()) %>% 
  filter(Final_status_historical== "Deleted") %>% 
  mutate(sample_size_deleted = nrow()) %>% 
  select(total_sample_size,sample_size_deleted )

@

<<Historical_completed_dataset_comparisons, eval=FALSE>>=
completed_subset <-
...
cancellation_by_project_stage <-
  ...
  mutate(n_first_status_possible_or_under_consideration = length(which(first_pipeline_status == "Possible or under consideration")),
         ...
         n_deleted_as_possible_or_under_consideration = length(which(final_pipeline_status == "Possible or under consideration" & Final_status_historical == "Deleted")),
         n_deleted_as_committted = length(which(final_pipeline_status == "Committed" & Final_status_historical == "Deleted")),
         n_deleted_as_under_construction = length(which(final_pipeline_status == "Under construction" & Final_status_historical == "Deleted")),
         ...
         overrall_cancellation_rate = length(which(Final_status_historical == "Deleted"))/length(Final_status_historical)) %>%
...
cancellation_and_cost_overruns <-
  ...
  merge(first_status_by_GR) %>%
  merge(final_status_by_GR) %>%
  mutate(cancellation_state = if_else(Final_status_historical== "Completed" , "None", as.character(final_pipeline_status)), 
...
# This code is defined relative to projects having status = committed, not ENTERING committed
grattan_records_180days_before_election <- 
  completed_transport_projects %>%
...
<<grattan_records_near_federal_election>>=
grattan_records_near_federal_election <- 
  date_first_committed_by_GR %>%
  merge(State_new_by_GR, by = "Grattan_record") %>%
  setkey(State_new, .Record_date.num) %>%
...
grattan_records_near_election <- 
  ...
  filter(election_flag) %>%
  .[["Grattan_record"]]

@

<<grattan_records_180days_before_state_election>>=
# This code is old and now incorrect
grattan_records_180days_before_state_election <- 
  completed_transport_projects %>%
  filter(Status_ordered == "Committed") %>%
  mutate(.Record_date.num = as.numeric(Record_date)) %>%
...
important_vars_by_GR <- 
  ...
  setkey(Grattan_record, Record_date) %>%
  group_by(Grattan_record) %>%
  summarise(premature_announcement = any(Status_ordered == "Possible or under consideration"), 
  ...
  mutate(log_First_cost_when_under_construction_real = if_else(is.na(log_First_cost_when_under_construction_real), 
         ...
         election_within_180_days = Grattan_record %in% grattan_records_near_election) 
@

<<fit_logit_co_all_v7>>=
important_vars_by_GR %>%
  merge(cost_overruns_by_GR) %>%
...
fit_logit_co_all_v7_mfx <- 
  important_vars_by_GR %>%
  merge(cost_overruns_by_GR) %>%
  logitmfx(fit_logit_co_all_v7, 
           data = .)

@

<<fit_logistic_co_all_v4>>=
important_vars_by_GR %>%
  merge(cost_overruns_by_GR) %>%
  filter(Final_cost_over_initial>1) %>%
  lm(Final_cost_over_initial ~ premature_announcement + 
          log_Total_days_pre_construction + 
          big_states + 
          Sub_industry + 
          Constructed_post_GFC + 
...
cost_overruns_by_GR_phase <-
  completed_transport_projects %>% 
  group_by(Grattan_record) %>%
  mutate(initial_project_cost = first(Cost_estimate_real)) %>%
  setkey(Grattan_record, Status_ordered, Record_date) %>%
  select(Grattan_record, Status_ordered, Record_date, Cost_estimate_real, initial_project_cost, cost_when_completed) %>% 
  group_by(Grattan_record, Status_ordered) %>%
  summarise(first_cost = first(Cost_estimate_real), 
            final_cost = last(Cost_estimate_real), 
            initial_cost = last(initial_project_cost)) %>%
  group_by(Grattan_record) %>%
  mutate(change_was_from = paste(lag(Status_ordered), "to", Status_ordered),
         prev_status_existed = !is.na(lag(Status_ordered))) %>%
  mutate(first_cost_change = first_cost - lag(first_cost), 
         final_cost_change = final_cost - lag(final_cost), 
         cost_change_over_stage = final_cost - first_cost) %>%
  ungroup %>%
  filter(prev_status_existed) %>%
  mutate(phase_cost_overrun = coalesce(first_cost_change, 0), 
         phase_cost_overrun_pc = phase_cost_overrun / initial_cost) %>%
  mutate(change_was_from = factor(change_was_from, 
                                    levels = c("Possible or under consideration to Committed", 
                                               "Possible or under consideration to Under construction", 
                                               "Possible or under consideration to Completed", 
  ...
  bind_rows({
    completed_transport_projects %>%
      ...
      summarise(first_estimate = first(Cost_estimate_real, order_by = Record_date), 
                ...
                initial_project_cost = last(initial_project_cost)) %>%
      ungroup %>%
      merge(cost_overruns_by_GR) %>%
      mutate(phase_cost_overrun = final_estimate - first_estimate, 
             phase_cost_overrun_pc = phase_cost_overrun / initial_project_cost) %>%
      ungroup %>%
...
<<other, eval=FALSE>>=
CO_announcement_commitment_LD <-
  cost_overruns_by_GR_phase %>%
  filter(change_was_from == "")
...
table(cost_overruns_by_GR_phase$change_was_from)

@

<<CO12_a_z_project_level>>=
CO12_a_z_project_level <-
  cost_overruns_by_GR_phase %>%
  filter(change_was_from == "Possible or under consideration to Committed") %>%
  setkey(Grattan_record) %>%
  unique %>%
  summarise(mean(phase_cost_overrun_pc) * Prop_projects_w_each_project_stage$prop_projects_w_possible_or_under_consideration)

@

<<CO23_a_z_project_level>>=
CO23_a_z_project_level <-
cost_overruns_by_GR_phase %>%
  filter(change_was_from == "Committed to Under construction") %>%
  setkey(Grattan_record) %>%
  unique %>%
  summarise(mean(phase_cost_overrun_pc)*Prop_projects_w_each_project_stage$prop_projects_w_committted)
@

<<CO34_a_z_project_level>>=
CO34_a_z_project_level <-
cost_overruns_by_GR_phase %>%
  filter(change_was_from == "Under construction to Completed") %>%
  setkey(Grattan_record) %>%
  unique %>%
  summarise(mean(phase_cost_overrun_pc))
@

<<CO_a_z_project_CHECK, eval = FALSE>>=
test_that("Check that indices are additive", {
  expect_equal(CO12_a_z_project_level + CO23_a_z_project_level + CO34_a_z_project_level - average_mag_of_CO, 0)
})
@

<<CO12_a_z_portfolio_level>>=
CO12_a_z_portfolio_level <- 
  cost_overruns_by_GR_phase %>%
  merge(cost_overruns_by_GR) %>%
  group_by(change_was_from) %>%
  summarise(x = sum(phase_cost_overrun) / sum(first_cost_estimate_real) ) %>%
  ungroup %>%
  filter(change_was_from == "Possible or under consideration to Committed") %$%
  sum(x*Prop_projects_w_each_project_stage$prop_projects_w_possible_or_under_consideration)
@

<<CO23_a_z_portfolio_level>>=
CO23_a_z_portfolio_level <- 
  cost_overruns_by_GR_phase %>%
  merge(cost_overruns_by_GR) %>%
  group_by(change_was_from) %>%
  ...
  ungroup %>%
  filter(change_was_from == "Committed to Under construction") %$%
  sum(x*Prop_projects_w_each_project_stage$prop_projects_w_committted)
@

<<CO34_a_z_portfolio_level>>=
CO34_a_z_portfolio_level <- 
  cost_overruns_by_GR_phase %>%
  merge(cost_overruns_by_GR) %>%
  group_by(change_was_from) %>%
...
<<CO_a_z_portfolio_CHECK, eval = FALSE>>=
if_else(CO12_a_z_portfolio_level+CO23_a_z_portfolio_level+CO34_a_z_portfolio_level- spending_per_dollar_budgeted ==0, "PASS: INDICES ARE ADDITIVE", "WARNING: INDICES ARE NOT ADDITIVE") 
@
...
overruns_during_construction_as_prop_of_final_value <- 
{
  ...
  merge(cost_overrun_during_construction_by_GR, cost_overruns_by_GR, by = "Grattan_record") %$%
    {sum(cost_overrun_during_construction) / sum(first_cost_estimate_real)}
}
@
...
overrun_underrun_net_by_Initial_status$Initial_status
@
