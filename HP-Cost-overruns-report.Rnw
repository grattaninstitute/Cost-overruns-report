...
table(cost_overruns_by_GR_phase$cost_change_over_stage, cost_overruns_by_GR_phase$change_was_from)
...
cost_overruns_by_GR_phase <-
  ...
  group_by(Grattan_record) %>%
  mutate(change_was_from = paste(Status_ordered, "to", lag(Status_ordered,k=-1))) %>%
