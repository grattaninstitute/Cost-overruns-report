...
CO_and_project_size_scatter <-
  ...
  filter(Final_cost_over_initial>0) %>%
  setkey(Grattan_record) %>%
  unique() %>%
  setkey(Grattan_record, Record_date) %>%
  group_by(Grattan_record) %>%
  mutate(project_size = first(Cost_estimate_real), 
         log_Final_cost_over_initial = if_else(Final_cost_over_initial>0, log(Final_cost_over_initial), 0)) %>%
  select(Grattan_record, project_size, Final_cost_over_initial)

plot(CO_and_project_size_scatter$project_size, CO_and_project_size_scatter$log_Final_cost_over_initial)
...
cost_overruns_by_project_size <-
  ...
  mutate(experienced_overrun = if_else(Final_cost_over_initial>1, 1, 0),
        Project_size_category = if_else(project_size<300000000, "lt 300m", if_else(project_size<600000000, "300m - 600m", "600+"))) %>%
  group_by(Project_size_category) %>%
  mutate(Mean_overrun = mean(Final_cost_over_initial), 
         n = n(), 
         pc_w_CO = mean(experienced_overrun)) %>%
  summarise(Project_size_category, Mean_overrun, pc_w_CO, n) %>%
  unique
...
prop_announced_early_which_completed <- 
  ...
  merge(final_status_by_GR) %>%
  filter(announced_prematurely, !is.na(Final_status)) %$%
  mean(Final_status == "Completed")
@
