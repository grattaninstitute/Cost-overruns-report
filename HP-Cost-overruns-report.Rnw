...
transport_projects <- 
  ...
  mutate(Grattan_record = if_else(is.na(`Record No`), 
                                  `Project No.`,
                                  `Record No`)) %>%
  ...
  mutate(Record_date = as.Date(paste("1", sheet), 
                               format = "%d %b %Y")) %>%
  mutate(Cost_estimate_real = construction_inflator(Cost_estimate, from = Record_date, to = as.Date("2015-12-01"))) %>%
...
savings_if_every_proj_with_CO_had_CO_le_10pc <-
  ...
  ungroup %>%
  filter(last_cost_estimate_real > 1.1 * first_cost_estimate_real) %>%
  ...
  sum(actual_overrun - overrun_threshold)
