...
savings_if_every_proj_with_CO_had_CO_le_10pc <-
  ...
  mutate(value_of_overrun_lt_10_pc = 0.1 * first_cost_estimate_real, 
         value_of_overrun_gt_10_pc = cost_overrun - value_of_overrun_lt_10_pc) %>%
  arrange(desc(value_of_overrun_gt_10_pc)) %$%
  # Assume every overrun is 10% (even if some were under)
  sum(value_of_overrun_gt_10_pc)
@
