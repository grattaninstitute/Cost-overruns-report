...
overrun_by_GR <- 
  ...
  summarise(last_cost_estimate_real = last(Cost_estimate_real, order_by = Record_date), 
            first_cost_estimate_real = first(Cost_estimate_real, order_by = Record_date)) %>%
  ungroup %>%
  mutate(overrun = last_cost_estimate_real - first_cost_estimate_real)
@
...
savings_if_every_proj_with_CO_had_CO_le_10pc <-
  ...
  mutate(overrun_threshold = 0.1 * first_cost_estimate_real, 
         overrun_cf_capped = overrun - overrun_threshold) %>%
  arrange(desc(overrun_cf_capped)) %$%
  # Assume every overrun is 10% (even if some were under)
...
prop_projects_on_budget <- 
  overrun_by_GR %$% 
  mean(last_cost_estimate_real <= first_cost_estimate_real)
@
...
val_projects_w_overrun_gt <- function(percentage_over){
  overrun_by_GR %>%
    mutate(is_over = overrun / first_cost_estimate_real > 1 + percentage_over) %$%
    sum(is_over * overrun)
}
@
...
pc_lower_ROI <- 
  ...
  {1 - (sum(first_cost_estimate_real) / sum(last_cost_estimate_real))}
@
...
spending_per_dollar_budgeted <- 
  ...
  setkey(Grattan_record, Record_date) %>%
  group_by(Grattan_record) %>%
  summarise(overrun_per_dollar = (last(Cost_estimate_real)) / first(Cost_estimate_real)) %$%
...
overrun_by_GR %>%
  mutate(overrun_prop = overrun / first_cost_estimate_real) %>%
  filter(overrun_prop > 0) %>%
  setorderv("overrun_prop") %>%
  mutate(prop_projects = (1:.N) / .N, 
         prop_value = cumsum(overrun) / sum(overrun)) %>%
  ...
  summarise(premature_announcement = any(Status_ordered <= "Under consideration"), 
            log_Total_days_pre_construction = 
              if (any(Status == "Under construction")){
                ...
                {
                  # Impute the value based total days before construction
                  completed_transport_projects %>% 
                    group_by(Grattan_record) %>%
                    filter(any(Status == "Under construction")) %>%
                    mutate(days_before_constr = difftime(min(Record_date[Status_ordered == "Under construction"]), 
                                                         min(Record_date), 
                                                         units = "days")) %$%
                    log(mean(as.numeric(days_before_constr)) + 1)
                }
              }, 
            ...
            Constructed_post_GFC = min(Record_date[Status_ordered == "Under construction"]) > as.Date("2008-09-12"), 
            log_First_cost_when_under_construction_real = log(first(Cost_estimate_real[Status_ordered == "Under construction"], 
                                                                    order_by = Record_date)), 
            # Due to violation of 3NF
            Sub_industry = first(`Sub-industry`)) %>%
  ungroup %>%
  mutate(election_within_180_days = Grattan_record %in% grattan_records_180days_before_election) %>%
  merge(overrun_by_GR) %>%
    glm(factor(overrun > 0) ~ premature_announcement + 
          log_Total_days_pre_construction + 
          big_states + 
          Sub_industry + 
...
fit_logit_co_all_v7_mfx <- 
  ...
  summarise(premature_announcement = any(Status_ordered <= "Under consideration"), 
            ...
            Constructed_post_GFC = if (any(Status_ordered == "Under construction")) min(Record_date[Status_ordered == "Under construction"]) > as.Date("2008-09-12") else FALSE, 
            log_First_cost_when_under_construction_real = log(first(Cost_estimate_real[Status_ordered == "Under construction"], 
                                                                    order_by = Record_date)), 
            # Due to violation of 3NF
            Sub_industry = first(`Sub-industry`)) %>%
  ...
  mutate(election_within_180_days = Grattan_record %in% grattan_records_180days_before_election) %>%
  merge(overrun_by_GR) %>%
  logitmfx(factor(overrun > 0) ~ premature_announcement + 
             log_Total_days_pre_construction + 
             big_states + 
             Sub_industry + 
             Constructed_post_GFC + 
...
waterfall_graph <- data.frame(Time_period = c("First cost announcement - budget commitment", 
                                              "Budget commitment - under construction", 
                                              "Under construction - completed", 
                                              "First cost announcement - completed"), 
                              Cost_overrun = c(co_all_table_1_portfolio_az$co12[3], 
                                               co_all_table_1_portfolio_az$co23[3], 
                                               co_all_table_1_portfolio_az$co34[3], 
                                               co_all_table_1_portfolio_az$co14[3]))
...
<<cost_overrun_size_by_proximity_to_state_election>>=
completed_transport_projects %>%
  group_by(Grattan_record) %>%
  summarise()
