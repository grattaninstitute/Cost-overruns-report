...
first_date_by_status <- 
  ...
  spread(Status_ordered, first_date) %>%
...
total_cost_overruns <- 
  completed_transport_projects_by_GR %>%
  summarise(sum(final_cost_estimate_real - first_cost_estimate_real))
@
...
savings_if_every_proj_with_CO_had_CO_le_10pc <-
  ...
  summarise(sum(value_of_overruns_gt_10_pc))
@
...
(prop_value_scope_changes %>% filter(Capacity_increase >0) %>%nrow)/(prop_value_scope_changes %>% nrow)
(completed_transport_projects %>% filter(!is.na(Capacity)) %>%nrow)/(completed_transport_projects %>% nrow)
View(completed_transport_projects$Capacity_mentioned)
...
<<rolling_join_test>>= # This is just a test.
{
...
<<grattan_records_near_election_LDs_attempt>>=
# This doesn't work yet..

gov_commitment_date<-
  first_date_by_status %>%
  mutate(gov_commitment_date =if_else(!is.na(Date_first_as_Committed),Date_first_as_Committed, 
                     if_else(!is.na(`Date_first_as_Under construction`),`Date_first_as_Under construction`, 
                             first(completed_transport_projects %>% group_by(Grattan_record)%$% Record_date)))) %>%
  summarise(Grattan_record, gov_commitment_date) %>%
  setkey(Grattan_record)

completed_transport_projects %>%
  merge(gov_commitment_date) %>%
  mutate(.gov_commitment_date.num = as.numeric(gov_commitment_date)) %>%
  setkey(State_new, .gov_commitment_date.num) %>%
  election_any_date[.,roll=-180] %>%
  filter(election_flag, is_federal) %$%
  unique(Grattan_record)
