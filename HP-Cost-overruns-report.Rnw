...
final_status_by_GR <- 
  ...
  select(Grattan_record = `Project No.`, 
         Status_according_to_historical_dataset = Status) %>%
  # See https://github.com/grattaninstitute/Cost-overruns-report/issues/20 
  right_join(transport_projects, by = "Grattan_record") %>%
  group_by(Grattan_record) %>%
  summarise(Final_status = 
              # i.e. it is not in historical_dataset:
              if (all(is.na(Status_according_to_historical_dataset))){
                if (any(Status_ordered == "Under construction") && year(max(Record_date)) < 2015){
                  "Completed"
                } else {
                  NA_character_
                }
...
sample_size_deleted <-
  final_status_by_GR %$% 
  sum(Final_status == "Deleted") 

sample_size_completed <-
  final_status_by_GR %$% 
  sum(Final_status == "Completed") 
@
