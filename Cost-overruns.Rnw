...
test_that("na_reset_rev expected", {
  expect_equal(na_reset_rev(c(1, 2, NA, NA, NA, 4, 5, NA, NA, 3, NA, 5)), 
               c(0L, 0L, 3L, 2L, 1L, 0L, 0L, 2L, 1L, 0L, 1L, 0L))
})
@
...
public_private_by_Grattan_record <- 
  ...
  mutate(is_public = as.logical(Public_project_Grattan)) %>%
  dplyr::select(Grattan_record, is_public) %>%
  as.data.table %>%
...
# Writing a variable for when project exits database
  ...
  mutate(Initial_date = min(Record_date), 
         ...
         Status = ifelse(Status == "Under Construction", "Under construction", Status),
...
# We therefore write a new variable for when the status of a project changes
  Status_change = is.na(Status != lag(Status, order_by = Qtrs_in_dataset)) | Status != lag(Status, order_by = Qtrs_in_dataset)
  )
...
final_cost_by_status <- 
  ...
  setnames(old = names(.)[names(.) != "Grattan_record"], 
           new = paste0("Final_cost_when_", names(.)[names(.) != "Grattan_record"])) %>% 
  setkey(Grattan_record)
...
project_dataset_plus %<>%
  mutate(Total_days_pre_construction = ifelse(is.na(Total_days_under_construction) == TRUE, 
                                              as.numeric(Final_date - Initial_date) - Average_days_under_construction, 
                                              as.numeric(`Date_first_as_Under construction` - Initial_date)),
...
project_dataset_plus <- 
  merge(project_dataset_plus, public_private_by_Grattan_record, all.x = TRUE)  
@
...
# D) Filter out projects with initial values below $20m:
...
cancellation_rate_by_cohort <-
  data.frame(First_project_stage = c("Possible and under consideration", "Committed", "Under construction"), 
...
co_all_table_1_portfolio_az <- 
  bind_rows("Road" = co_all_road_indices_AM_portfolio, 
            "Rail" = co_all_rail_indices_AM_portfolio, 
            "All" = co_all_indices_AM_portfolio, .id = "Type")
...
#Key figures:
n_projects <- project_dataset_t %>% nrow
n_completed_projects <- co_all %>% nrow
prop_g1_projects_completed <- 
  project_dataset_t_g1 %$% 
  mean(Final_status_historical == "Completed")
@

<<savings_if_every_proj_with_CO_had_CO_le_10pc>>=
savings_if_every_proj_with_CO_had_CO_le_10pc <- 
  savings_from_no_co_gt_10_pc <- 
  co_all %>%
...
prop_projects_w_overrun <- 
  co_all %$% 
  mean(co_initial_final_category == "Cost overrun")
@

<<prop_projects_w_overrun_gt_0.5>>=
prop_projects_w_overrun_gt_0.5 <- 
  co_all %$% 
  mean(Final_cost_over_initial > 1.5)
@

<<prop_co_value_from_projects_w_overrun_gt_0.5>>=
prop_projects_w_overrun_gt_0.5 <- 
  (sum(overrun_gt_50_pc$Final_cost_real) - sum(overrun_gt_50_pc$Initial_cost_real)) / 
  (sum(overruns$Final_cost_real) - sum(overruns$Initial_cost_real))
@

<<prop_projects_w_overrun_gt_2>>=
prop_projects_w_overrun_gt_2 <- 
  co_all %$%
  mean(Final_cost_over_initial > 3)
@

<<prop_co_value_projects_gt_2>>=
prop_co_value_projects_gt_2 <-
  (sum(huge_overruns$Final_cost_real) - sum(huge_overruns$Initial_cost_real)) / 
  (sum(overruns$Final_cost_real) - sum(overruns$Initial_cost_real))
@
...
value_B_of_co_gt_2 <- 
  # data is in millions
  (sum(huge_overruns$Final_cost_real) - sum(huge_overruns$Initial_cost_real)) / 1000
@

<<spending_per_dollar_budgeted>>=
spending_per_dollar_budgeted <- 
  co_all %$%
  {sum(Final_cost_real) / sum(Initial_cost_real)}
@
...
pc_lower_ROI <- 
  ...
  {1 - (sum(Initial_cost_real) / sum(Final_cost_real))}
@

<<cost_of_overruns_gt_50_pc>>=
cost_of_overruns_gt_50_pc <- 
  overrun_gt_50_pc %$%
  {sum(Final_cost_real) - sum(Initial_cost_real)}
...
<<source-continued>>=
<<<<<<< HEAD
SS_coall_table_10_variable_names <-
  data_frame(Variable = c("n_completed_projects", 
                          "savings_from_no_co_gt_10_pc", 
                          "prop_projects_on_budget", 
                          "prop_projects_w_overrun", 
                          "spending_per_dollar_budgeted", 
...
SS_coall_table_10_content <- 
  as.data.frame(c(n_completed_projects, 
                  data.frame(savings_from_no_co_gt_10_pc), 
                  prop_projects_on_budget, 
                  prop_projects_w_overrun, 
                  data.frame(spending_per_dollar_budgeted), 
                  data.frame(pc_lower_ROI),
...
value_B_of_co_gt_0.5<- (sum(overrun_gt_50_pc$Final_cost_real)-sum(overrun_gt_50_pc$Initial_cost_real))/1000
...
<<exploratory, eval=TRUE>>=
#* Exporting misc subsets of the data (This is exploratory, not needed for the report): ----
the_worst_offenders <-
  co_all %>%
  filter(Final_cost_over_initial>2) %>%
  select(Grattan_record, Project, Final_cost_over_initial, Initial_cost, First_cost_when_Possible_or_under_consideration, First_cost_when_Committed, First_cost_when_under_construction, Final_cost) %>%
  arrange(Final_cost_over_initial)

project_list_for_Eric <-
  co_all %>%
  filter(Final_cost>1000) %>%
  select(Project, Final_cost, Final_cost_real) %>%
  arrange(Final_cost)
...
# Regression
fit_logit_co_all_v7 <- 
    glm(experienced_overrun ~ premature_announcement + 
          log_Total_days_pre_construction + 
          big_states + 
          `Sub-industry` + 
          `Constructed_post_GFC`+ 
          log(First_cost_when_under_construction_real) + 
          election_180_days, 
        data = co_all, 
...
# All
# Regression
fit_logistic_co_all_v4 <- lm(log(Final_cost_over_initial) ~ premature_announcement + log_Total_days_pre_construction   
                             + big_states + 
                               `Sub-industry` +
                               `Constructed_post_GFC`+ 
                               log(First_cost_when_under_construction_real)  + election_180_days, 
                             data = co_positive_only_all)
...
#Preditcted probabilities, pre post GFC graph - graph for report: 
  #Marginal effects
  logit_marginal_effects <- c(as.numeric(marginal_effects_logit_co_all_v7$mfxest[1]), as.numeric(marginal_effects_logit_co_all_v7$mfxest[2]), as.numeric(marginal_effects_logit_co_all_v7$mfxest[3]), 
                            as.numeric(marginal_effects_logit_co_all_v7$mfxest[4]),as.numeric(marginal_effects_logit_co_all_v7$mfxest[5]), as.numeric(marginal_effects_logit_co_all_v7$mfxest[6]), 
                            as.numeric(marginal_effects_logit_co_all_v7$mfxest[7]), as.numeric(marginal_effects_logit_co_all_v7$mfxest[8]), as.numeric(marginal_effects_logit_co_all_v7$mfxest[9]), 
                            as.numeric(marginal_effects_logit_co_all_v7$mfxest[10]))
  
  # Dependent variables 
  pc_QLD <- (co_all %>% filter(State_new == "Queensland") %>% nrow()/nrow(co_all))
  pc_VIC <- (co_all %>% filter(State_new == "Victoria") %>% nrow()/nrow(co_all))
  pc_NSW <- (co_all %>% filter(State_new == "New South Wales") %>% nrow()/nrow(co_all))
  pc_WA <- (co_all %>% filter(State_new == "Western Australia") %>% nrow()/nrow(co_all))
  pc_small_states <- 1-pc_QLD-pc_VIC-pc_NSW-pc_WA
  
  dep_var_means_pre_GFC <- c(nrow(co_g1)/nrow(co_all), mean(co_all$log_Total_days_pre_construction), pc_QLD, pc_small_states, pc_VIC, pc_WA, mean(co_all$Road),
                     0, mean(log(co_all$First_cost_when_under_construction)), mean(co_all$election_180_days))
  
  dep_var_means_post_GFC <- c(nrow(co_g1)/nrow(co_all), mean(co_all$log_Total_days_pre_construction), pc_QLD, pc_small_states, pc_VIC, pc_WA, mean(co_all$Road),
                     1, mean(log(co_all$First_cost_when_under_construction)), mean(co_all$election_180_days))
  
  #Predicted probabilities
  predicted_CO_pre_GFC <- sum(dep_var_means_pre_GFC*logit_marginal_effects)
  predicted_CO_post_GFC <- sum(dep_var_means_post_GFC*logit_marginal_effects)
...
The following three sections identify three major places where improvements could be made so as to save a substantial share of the $\Sexpr{round(sum(co_all$Final_cost - co_all$First_cost_when_Committed_a_z)/1000,0)} billion of overruns occurring after the formal funding commitment. 
