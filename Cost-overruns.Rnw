...
savings_B_from_no_co_gt_10_pc <- 
  big_overruns %$%
  # If every cost-overrun was only 10%, subtract from
  # initial_cost_real
  sum(Final_cost_real - Initial_cost_real * 1.1) %>%
...
\begin{overview}[-40pt]
Most of the time, Australia isn’t bad at managing transport infrastructure projects. But when one goes wrong, it can be spectacular. Australia could have saved \Sexpr{texNum(savings_B_from_no_co_gt_10_pc, dollar = TRUE)} over the past 15 years if every transport infrastructure project with a cost overrun had gone no more than ten per cent over budget. 
