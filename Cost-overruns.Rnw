...
if_else(sum(project_dataset_plus$Initial_cost) - sum(project_dataset_plus$Initial_cost_real) ==0, "Stop and check this: inflator isn't working", "Good: real costs are different from nominal costs")
...
co_all_table_3 <-  
  co_all %>%
  arrange(Matching_status, co_initial_final_category) %>%
...
overall_overruns_by_cohort <- data.frame(variable = c("Possible or under consid cohort", "Committed cohort", "Under construction cohort", "Committed and under construction cohorts"),
                                           Net_cost_overrun = c(sum(co_g1$Final_cost_real)/sum(co_g1$Initial_cost_real), sum(co_g2$Final_cost_real)/sum(co_g2$Initial_cost_real), sum(co_g3$Final_cost_real)/sum(co_g3$Initial_cost_real), sum(co_g2g3$Final_cost_real)/sum(co_g2g3$Initial_cost_real)),
                                         Total_cost_overrun = c(sum(co_g1_overruns_only$Final_cost_real)/sum(co_g1_overruns_only$Initial_cost_real), sum(co_g2_overruns_only$Final_cost_real)/sum(co_g2_overruns_only$Initial_cost_real), sum(co_g3_overruns_only$Final_cost_real)/sum(co_g3_overruns_only$Initial_cost_real), sum(co_g2g3_overruns_only$Final_cost_real)/sum(co_g2g3_overruns_only$Initial_cost_real)))  
...
savings_B_from_no_co_gt_10_pc<- sum(big_overruns$Final_cost_real)-sum(big_overruns$Initial_cost_real)*1.1/1000  
pc_of_projects_on_budget<- (co_all %>% filter(co_initial_final_category =="On budget") %>% summarise(n()))/(co_all %>% summarise(n()))
...
\begin{overview}[-40pt]
Most of the time, Australia isn’t bad at managing transport infrastructure projects. But when one goes wrong, it can be spectacular. Australia could have saved \Sexpr{savings_B_from_no_co_gt_10_pc} billion over the past 15 years if every transport infrastructure project with a cost overrun had gone no more than ten per cent over budget. 

Policymakers appear to be complacent about this outcome. At first glance, this is understandable: \Sexpr{pc_of_projects_on_budget} per cent of projects finish on budget, and we are assured that every extreme cost blowout comes with its own story. 
...
Over the last 15 years, Australia has spent at least $\Sexpr{spending_per_dollar_budgeted} for every dollar initially budgeted for transport infrastructure projects. Cost overruns on infrastructure projects are not a uniquely Australian phenomenon, but Australia’s cost overruns – especially those that occur prior to construction – are larger than is commonly observed on similarly sized projects overseas. 
...
Cost overruns on transport infrastructure projects are nothing new. Some of the biggest examples are widely known, such as Western Australia’s Forrest Highway between Perth and Bunbury, which cost nearly six times its initial budget, or New South Wales’ Hunter Expressway, which cost nearly four times its initial budget. The amount of money at stake is very large: transport infrastructure over the past 15 years has cost $\Sexpr{savings_B_from_no_co_gt_10_pc} billion more than taxpayers were told it would. 
...
...But the frequency of overruns is only a small part of the story: what is much more important is the size. While only \Sexpr{pc_of_projects_w_overrun_gt_0.5} per cent of projects overran their budget by 50 per cent or more, such overruns accounted for \Sexpr{pc_of_co_value_from_projects_w_overrun_gt_0.5} per cent of the total value of overruns (Figure xx). The cost to government budgets of overruns of 50 per cent or more was $xx billion over the past fifteen years.

Even bigger blowouts, of 200 per cent or more, occurred on \Sexpr{pc_of_projects_w_overrun_gt_2} per cent of projects, but accounted for fully a quarter (\Sexpr{pc_of_co_value_from_projects_w_overrun_gt_2}\%)of the total value of all overruns. The cost of overruns where the project was completed at double its budgeted cost or more was $\Sexpr{value_B_of_co_gt_2} billion.
...
...some text...
Scope changes only account for around 12 per cent of cost overruns on transport infrastructure projects, even including the full project lifecycle from the time of first announcement HUGH: This figure comes from the Grattan dataset. The analysis is in excel. 

Figure 2.3.1: Scope changes pie chart   
...some text...
...
HUGH: This finding comes the eight regression models. See the Regression results final spreadsheet

\subsection{Early overruns do not prevent later overruns}
Cost overruns occur throughout the project lifecycle, accounting overall for \Sexpr{spending_per_dollar_budgeted-1}  cents additional cost for every dollar of estimated project cost (Figure 2.3.3). 

Overruns in the initial phase, between announcement and before a formal funding commitment, occur at a rate of \Sexpr{co_all_indices_AM$co12} cents on every dollar of project value. These overruns are concerning because any later overruns that occur do so on the enlarged cost base resulting from the earlier overrun. 
Overruns in the middle phase, after the funding commitment up until construction begins, occur at a rate of \Sexpr{co_all_indices_AM$co23} cents on every dollar of project value. Overruns during construction occur at a rate of \Sexpr{co_all_indices_AM$co34} cents for every dollar of project value. These middle and later phase overruns are concerning because they reveal that the initial decision to invest was made on an inaccurate basis. 

Figure 2.3.3

The inaccurate basis for the decision to invest is also apparent from analysis of the cost estimates. Cost estimates produced to a standard that anticipates the project will equal or better its budget three quarters of the time (known as a “P75” costing) actually only achieved this result \Sexpr{1-pc_of_projects_w_overrun} per cent of the time. The difference reveals significant understatement of costs across the portfolio of projects planned or built since 2000.

\chapter{Cost overruns are more preventable than people think}
..some text..

\section {Poor incentives for decision-makers}
Ministers, governments and would-be governments have always sought political advantage by promising infrastructure. But politicised announcements that ignore proper process have poor outcomes. Cost overruns are 36 per cent larger on average for projects announced close to a state election than otherwise similar projects announced at other times. This confirms the contention of previous Grattan report, Roads to Riches, that politicians commit to poor quality projects for political benefit.  
HUGH: See Regression results final spreadsheet

\section{Premature announcements}

NOTE: I think Marion was referring to the gross figures in her text. This is not clear,and I don't think the right numbers to cite. The figures below are net of underruns:
Premature announcement affects not only the likelihood of an overrun, but also its size. Projects with costs announced ahead of a government funding commitment have far larger cost overruns, averaging \Sexpr{overall_overruns_by_cohort[1,2]-1} per cent of the initially announced cost (Figure xx). By contrast, those announced with a formal funding commitment averaged overruns of \Sexpr{overall_overruns_by_cohort[4,2]-1} per cent.

When the net impact of cost overruns and under-runs is closer to zero, this indicates that the variation is less to do with preventable errors and more a case of normal variation in project cost outcomes relative to estimates. Figure xx illustrates net overruns of \Sexpr{overall_overruns_by_cohort[2,2]-1} to \Sexpr{overall_overruns_by_cohort[3,2]-1} per cent occur for projects announced with more mature cost estimates than those announced early.
