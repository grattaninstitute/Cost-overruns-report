...
<<source_all, echo=FALSE, message=FALSE>>=
...
overall_overruns_by_cohort <- data.frame(variable = c("Possible or under consid cohort", "Committed cohort", "Under construction cohort"),
                                           Net_cost_overrun = c(sum(co_g1$Final_cost_real)/sum(co_g1$Initial_cost_real), sum(co_g2$Final_cost_real)/sum(co_g2$Initial_cost_real), sum(co_g3$Final_cost_real)/sum(co_g3$Initial_cost_real)),
                                         Total_cost_overrun = c(sum(co_g1_overruns_only$Final_cost_real)/sum(co_g1_overruns_only$Initial_cost_real), sum(co_g2_overruns_only$Final_cost_real)/sum(co_g2_overruns_only$Initial_cost_real), sum(co_g3_overruns_only$Final_cost_real)/sum(co_g3_overruns_only$Initial_cost_real)))  
...
big_overruns <-
  co_all %>%
  filter(Final_cost_over_initial>=1.1)
...
savings_from_no_co_gt_10_pc<- sum(big_overruns$Final_cost_real)-sum(big_overruns$Initial_cost_real)*1.1  
...
\begin{overview}[-40pt]
Most of the time, Australia isn’t too bad at managing transport infrastructure projects. But when they go wrong, it can be spectacular. Australia could have saved $33 billion over the past 15 years if every transport infrastructure project with a cost overrun had overrun only within a ‘reasonable’ range of no more than ten per cent over budget. 

Policymakers appear to be complacent about this outcome. At first glance, this is understandable: 64 per cent of projects finish on budget, and we are assured that every extreme cost blowout comes with its own story. 

However, data on the cost overruns incurred over the 688 $20m+ transport infrastructure projects built in Australia since 2000 reveals that these anecdotes are misleading: Australia’s cost overruns are larger and far more common than could be considered ‘normal’, and more predictable than their surprised proponents realise. 

Over the last 15 years, Australia has spent at least $1.33 for every dollar initially budgeted for transport infrastructure projects. Cost overruns on infrastructure projects are not a uniquely Australian phenomenon, but Australia’s cost overruns – especially those that occur prior to construction – are larger than is commonly observed on similarly sized projects. 
...
These efforts are not proportionate to the importance of cost management on transport infrastructure projects. Australia has spent more on transport infrastructure over the last 15 years than we’ve saved in our Future Fund, and the realisation of a 32 per cent lower return on investment than promised in final business cases on average represents a substantial distortion to infrastructure investment decision making. 

Australia’s toothless appraisal process is not assisted by the perverse incentives facing project proponents. When cheaper projects are more likely to get up, top-up federal funding relieves proponents from facing the marginal cost of overruns and projects’ scopes can be readily expanded during construction, it is not surprising that cost overruns occur far more commonly than underruns. 

Moreover, the efforts Australia’s made to manage cost overruns are not proportional to the opportunity to anticipate and manage cost overruns better. Our analysis identifies that the average size of cost overruns at each project stage varies alongside project size, maturity, mode, the project’s political circumstances and the size of earlier overruns. Insofar as cost overruns can be predicted by observable characteristics like these, opportunities exist for data driven policy that anticipates and actively manages uncertainty.
